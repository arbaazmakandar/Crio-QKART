  ...
  return (
    <Box display="flex" flexDirection="column">
      <TextField
        ...
        onChange = {(event)=>handleNewAddress((currNewAddress) => ({
                      ...currNewAddress,
    ...
    if(balance < totalCartValue){
      ...
      try {
        ...
        if(response.status === 200){
          balance = balance - totalCartValue;
        ...
        if (e.response) {
          ...
          enqueueSnackbar(
            "Could not perform this checkout. Check that the backend is running, reachable and returns valid JSON.",
            {
              variant: "error",
            }
          );
              ...
              {/* TODO: CRIO_TASK_MODULE_CHECKOUT - Display list of addresses and corresponding "Delete" buttons, if present, of which 1 can be selected */}
               ...
               {(addresses!==0) ? <>
                  ...
                  {addresses.all.map((address) =>
                    ...
                    (
                        <Box key={address._id} onClick={()=>handleSelectedAddress(address._id)} className={`address-item ${addresses.selected===address._id ? "selected" : "not-selected"}`}>
